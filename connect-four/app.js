document.addEventListener('DOMContentLoaded', () => {
    ...
    ((index) => {
        // Add an onclick to each square of the grid
        squares[i].onclick = () => {
            // If the square below of the current one is taken, player can go on top of it
            if (squares[index + 7].classList.contains('taken')) {
                ...
                //if the square below the current one is not taken, player can't go there
            }
            else
                alert("Can't go here!");
            checkBoard();
    ...
    function checkBoard() {
        ...
        //Now take the the 4 values in each winningArrays and plug them into the squares
        for (let y = 0; y < winningArrays.length; y++) {
            ...
            if (square1.classList.contains('player-one') &&
                ...
                //If so, display player one as the winner
